...
var flow = {
    ...
    detectState: function () {
        ...
        if (allText.indexOf("签退") >= 0 || (allText.indexOf("上班") >= 0 && allText.indexOf("打卡时间") >= 0)) return "logged_in";

        if (allText.indexOf(config.text.checkin) >= 0) return "attendance";

        if (allText.indexOf(config.text.smsLogin) >= 0 && /\d{1,2}\s*s/.test(allText)) return "code_countdown";
    ...
    run: function () {
        ...
        try {
            ...
            if (state == "login_phone" || state == "code_input" || state == "code_countdown" || state == "unknown") {
