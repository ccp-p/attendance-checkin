...
var flow = {
    ...
    // ???????????????(u2 dump????,AutoJS???)
    dismissLocationPopup: function () {
        var errNode = text(config.text.locationError).findOne(2000);
        if (!errNode) return false;
        logger.warn("?????????????,??????");
        var confNode = text(config.text.confirm).findOne(5000);
        if (confNode) {
            var b = confNode.bounds();
            click(b.centerX(), b.centerY());
            logger.ok("??????????? " + b.centerX() + "," + b.centerY());
            sleep(2000);
            return true;
        }
        logger.error("???????????????");
    ...
    doCheckin: function () {
        logger.info("????/????(webview ??????)");
        this.dismissLocationPopup();
        var maxRetry = 6;
        var labels = [config.text.checkin, config.text.checkout];
        for (var i = 0; i < maxRetry; i++) {
            for (var j = 0; j < labels.length; j++) {
                var node = text(labels[j]).findOne(5000);
                if (node) {
                    var b = node.bounds();
                    click(b.centerX(), b.centerY());
                    logger.info("??" + labels[j] + "??: " + b.centerX() + "," + b.centerY() + " (?" + (i+1) + "???)");
                    sleep(config.timeout.pageLoad);
                    return true;
                }
            }
    ...
    // 步骤6: 点击「获取验证码」(点击后轮询验证按钮已变倒数,确认短信已触发)
    requestCode: function () {
        logger.info("点击获取验证码");
        if (!ui.clickAny(config.text.getCode, config.timeout.findElement)) {
            logger.error("获取验证码按钮未找到,可能已是登录态");
            return false;
        }
        // 点击后按钮应变为倒数(如 59s / 60秒后重获),轮询确认
        for (var i = 0; i < 10; i++) {
            sleep(1000);
            var texts = ui.collectAllTexts();
            for (var j = 0; j < texts.length; j++) {
                var t = texts[j];
                if (/\d{1,2}\s*s/.test(t) || t.indexOf("重新获取") >= 0 || t.indexOf("重发") >= 0) {
                    logger.ok("获取验证码成功,按钮已变倒数: " + t);
                    return true;
                }
            }
            logger.info("等待按钮变倒数... (第" + (i+1) + "次)");
        }
        logger.error("获取验证码后未检测到倒数,可能未触发发送");
        return false;
    },

    // 等待指定文字出现(通用轮询,webview 加载慢时用)
    waitForText: function (txt, timeoutMs, label) {
        label = label || txt;
        logger.info("等待[" + label + "]出现...");
        var deadline = Date.now() + (timeoutMs || 15000);
        while (Date.now() < deadline) {
            var texts = ui.collectAllTexts();
            if (texts.join(" ").indexOf(txt) >= 0) {
                logger.info("[" + label + "]已出现");
                return true;
            }
            sleep(1000);
        }
        logger.warn("[" + label + "]未在超时内出现");
        return false;
    },

    // ??????????????
    waitForAny: function (textArr, timeoutMs, label) {
        label = label || textArr.join("/");
        logger.info("??[" + label + "]??...");
        var deadline = Date.now() + (timeoutMs || 15000);
        while (Date.now() < deadline) {
            var texts = ui.collectAllTexts();
            ...
            for (var i = 0; i < textArr.length; i++) {
                if (allText.indexOf(textArr[i]) >= 0) {
                    logger.info("[" + label + "]???(" + textArr[i] + ")");
                    return true;
                }
            }
            sleep(1000);
    ...
    // 步骤10: 检测并处理「可信认证」弹窗(随机出现,webview)
        // 检测并处理「可信认证」弹窗(随机出现, webview)
    // 取消按钮坐标: bounds=[69,1770][1011,1899] 中心 (540,1834)
    handleTrustedAuth: function () {
        ...
        for (var i = 0; i < 6; i++) {
            if (this.isTrustedAuth()) {
                ...
                sleep(2000);
                if (this.isTrustedAuth()) {
                    logger.warn("取消未生效, 坐标回退 (540,1834)");
                    click(540, 1834);
                    sleep(2000);
                }
                if (!this.isTrustedAuth()) {
    ...
    // 轻量级检测: 当前是否在可信认证页面
    isTrustedAuth: function () {
        var texts = ui.collectAllTexts();
        var allText = texts.join(" ");
        return allText.indexOf(config.text.trustedAuth) >= 0 ||
               allText.indexOf(config.text.trustedAuthPlatform) >= 0;
    },

    // 点击取消按钮: 文字匹配 -> 包含 -> 坐标回退
    clickCancel: function () {
        var node = text(config.text.cancel).findOne(3000);
        if (node) {
            var b = node.bounds();
            click(b.centerX(), b.centerY());
            logger.ok("点击取消(文字) " + b.centerX() + "," + b.centerY());
            return true;
        }
        node = textContains(config.text.cancel).findOne(3000);
        if (node) {
            var b2 = node.bounds();
            click(b2.centerX(), b2.centerY());
            logger.ok("点击取消(包含) " + b2.centerX() + "," + b2.centerY());
            return true;
        }
        click(540, 1834);
        logger.ok("点击取消(坐标) 540,1834");
        return true;
    },
    ...
    detectState: function () {
        ...
        if (allText.indexOf(config.text.checkin) >= 0 || allText.indexOf(config.text.checkout) >= 0) return "attendance";
    ...
    run: function () {
        ...
        try {
            ...
            if (state == "home") {
                ...
                this.waitForText(config.text.attendance, 10000, "考勤打卡入口");
                state = this.detectState();
            }
            ...
            if (state == "workbench") {
                ...
                this.checkTrusted();
                this.waitForAny([config.text.checkin, config.text.checkout], 15000, "??/????");
                state = this.detectState();
            }
